...
public class LdapEmployeeDao {
    ...
    public List<Employee> findAll(String base, String objectclass) {
        ldapTemplate.setIgnorePartialResultException(true);
        List<Employee> employees =
                ldapTemplate.search(base, "(objectclass=person)", new MyAttributesMapper());
        return employees;
    }
    ...
    private static class MyAttributesMapper implements AttributesMapper<Employee> {
        public Employee mapFromAttributes(Attributes attributes) throws javax.naming.NamingException {
            ...
            employee.setEntryTime(
                    msExchWhenMailboxCreated == null
                            ? null
                            : ((String) msExchWhenMailboxCreated.get()).substring(0, 8));

            Attribute physicalDeliveryOfficeName = attributes.get("physicalDeliveryOfficeName");
            employee.setLeaveTime(
                    physicalDeliveryOfficeName == null ? null : (String) physicalDeliveryOfficeName.get());
